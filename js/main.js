...
// Dropdown functionality for nested dropbars in the navigation
document.addEventListener('DOMContentLoaded', function() {
    // Helper to show/hide with transition
    ...
    // Helper to check if mouse truly left the dropdown or its button
    function isLeavingToChild(e, containers) {
        if (!e.relatedTarget) return false;
        for (let i = 0; i < containers.length; i++) {
            if (containers[i].contains(e.relatedTarget)) return true;
        }
        return false;
    }
    ...
    // Parent dropbar
    var parentDropdown = document.querySelector('.dropdown-navbar > .dropbtn');
    var parentMenu = document.querySelector('.dropdown-navbar > .dropdown-content-navbar');
    if (parentDropdown && parentMenu) {
        var parentContainer = parentDropdown.parentElement;
        var parentRelated = [parentContainer, parentDropdown, parentMenu];

        // Desktop: hover
        parentContainer.addEventListener('mouseenter', function() {
            if (!isMobile()) showMenu(parentMenu);
        });
        parentContainer.addEventListener('mouseleave', function(e) {
            if (!isMobile() && !isLeavingToChild(e, parentRelated)) {
                hideMenu(parentMenu);
            }
        });
        parentDropdown.addEventListener('mouseleave', function(e) {
            if (!isMobile() && !isLeavingToChild(e, parentRelated)) {
                hideMenu(parentMenu);
            }
        });
        ...
        parentMenu.addEventListener('mouseleave', function(e) {
            if (!isMobile() && !isLeavingToChild(e, parentRelated)) {
                hideMenu(parentMenu);
            }
        });
        ...
        parentDropdown.addEventListener('click', function(e) {
            if (isMobile()) {
                e.preventDefault();
                if (parentMenu.style.display === 'block') {
                    hideMenu(parentMenu);
                } else {
                    showMenu(parentMenu);
                }
    ...
    // Nested dropbar
    var nestedDropdowns = document.querySelectorAll('.dropdown-navbar .dropdown-navbar');
    nestedDropdowns.forEach(function(nested) {
        var btn = nested.querySelector('.dropbtn');
        var menu = nested.querySelector('.dropdown-content-navbar');
        if (btn && menu) {
            var nestedRelated = [nested, btn, menu];

            // Desktop: hover
            nested.addEventListener('mouseenter', function() {
                if (!isMobile()) showMenu(menu);
            });
            nested.addEventListener('mouseleave', function(e) {
                if (!isMobile() && !isLeavingToChild(e, nestedRelated)) {
                    hideMenu(menu);
                }
            });
    ...
    // Optional: Hide all dropdowns when resizing from mobile to desktop
    window.addEventListener('resize', function() {
        if (!isMobile()) {
            // Hide all dropdown-content-navbar
            document.querySelectorAll('.dropdown-content-navbar').forEach(function(menu) {
                menu.style.display = '';
                menu.style.opacity = '';
